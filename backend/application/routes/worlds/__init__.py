...
async def get_worlds(last: str = None, limit: int = 100):
    ...
    if not results.count > 0:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No worlds found."
        )
...
async def get_world(world_id: str):
    ...
    return JSONResponse(content={**world, "snapshots": results.items})
...
async def find_world(world_name: str):
    ...
    if not results.count > 0:
        return HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"World: {world_name} does not exist.",
        )
...
async def delete_world(world_id: str):
    ...
    return JSONResponse(
        status_code=200, content=f"World: {world_id} deleted successfully."
    )
